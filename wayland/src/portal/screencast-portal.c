...
static void on_pipewire_remote_opened_cb(GObject *source, GAsyncResult *res, void *user_data) {
    struct ScreencastPortalCapture *capture;
    ...
    capture->stream_ready_cb(pipewire_fd);
    /*
...
static void open_pipewire_remote(struct ScreencastPortalCapture *capture) {
...
static void on_start_response_received_cb(GVariant *parameters, void *user_data) {
    struct ScreencastPortalCapture *capture = user_data;
    ...
    if (get_screencast_version() >= 4) {
        ...
        if (restore_token) capture->restore_token = bstrdup(g_variant_get_string(restore_token, NULL));

        // obs_source_save(capture->source);
    }

    blog(LOG_INFO, "[pipewire] %s selected, setting up screencast", capture_type_to_string(capture->capture_type));
...
static void start(struct ScreencastPortalCapture *capture) {
    GVariantBuilder builder;
    char *request_token;
    char *request_path;
...
static void select_source(struct ScreencastPortalCapture *capture) {
    GVariantBuilder builder;
    uint32_t available_cursor_modes;
    char *request_token;
    char *request_path;
    ...
    if (available_cursor_modes & PORTAL_CURSOR_MODE_METADATA)
        g_variant_builder_add(&builder, "{sv}", "cursor_mode", g_variant_new_uint32(PORTAL_CURSOR_MODE_METADATA));
    ...
    if (get_screencast_version() >= 4) {
        ...
        if (capture->restore_token && *capture->restore_token) {
            g_variant_builder_add(&builder, "{sv}", "restore_token", g_variant_new_string(capture->restore_token));
        }
    }
...
static void create_session(struct ScreencastPortalCapture *capture) {
    GVariantBuilder builder;
    char *session_token;
    char *request_token;
    char *request_path;
                              ...
                              obs_property_t *property, void *data)
{
        ...
        struct ScreencastPortalCapture *capture = data;
                                                      ...
                                                      obs_source_t *source)
{
        ...
        capture = bzalloc(sizeof(struct ScreencastPortalCapture));
