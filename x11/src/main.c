...
gchar* compose_gstreamer_pipeline() {
    ...
    if (VIDEO_ENC == "gpu-intel") {
        ...
        // stream-format=byte-stream breaks nvidia pipeline
    ...
    // for nvidia-gpu:            packetization-mode=1;profile-level-id=42e01f;level-asymmetry-allowed=1
    // which are compatible with both chrome and firefox
    // baseline and main profiles generate profile-level-ids which are not supported by Firefox in cpu and intel-gpu
    // modes.
