...
gchar *compose_gstreamer_pipeline() {
    ...
    if (VIDEO_ENC == "gpu-intel") {
        ...
        pipeline_str = g_strconcat(pipeline_str,
            ...
            ! nvh264enc name=nvenc bitrate=10000 rc-mode=cbr gop-size=-1 strict-gop=true aud=false b-adapt=false rc-lookahead=0 bframes=0 zero-reorder-delay=true cabac=true repeat-sequence-header=true preset=p4 tune=ultra-low-latency multipass=two-pass-quarter",
                                   NULL);
